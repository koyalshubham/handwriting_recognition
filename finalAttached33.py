...
def convert_scroll_to_imgs():
    ...
    segmented_char_mean_path = 'segmented_characters_mean'
    segmented_char_st_dev_path = 'segmented_characters_st_dev'
    ...
    directories = [scroll_path, holes_path, binary_path, marked_areas_path, segmented_areas_path,
                   ...
                   dilation_path, segmented_char_path, marked_char_path,
                   segmented_areas_path_trash,
                   segmented_char_mean_path, segmented_char_st_dev_path]
    ...
    for scroll in images:
        ...
        if not scroll.startswith('.'):
            ...
            if not os.path.exists(segmented_char_mean_path + '/' + folder):
                os.mkdir(segmented_char_mean_path + '/' + folder)
                print("Directory ", segmented_char_mean_path + '/' + folder, " created ")

            if not os.path.exists(segmented_char_st_dev_path + '/' + folder):
                os.mkdir(segmented_char_st_dev_path + '/' + folder)
                print("Directory ", segmented_char_st_dev_path + '/' + folder, " created ")
            ...
            for component in zip(ctrs, hierarchy):
                ...
                if currentHierarchy[3] < 0:  # these are the outermost parent components
                    ...
                    if h <= min_height and w <= min_width:  # if image is noise
                        cv2.imwrite(
                            segmented_areas_path_trash + '/' + folder + '/line ' + str(line) + ' col ' + str(
                                x) + '.jpg',
                            roi)
                        ...
                        # Save segments
                        cv2.imwrite(
                            segmented_areas_path + '/' + folder + '/line ' + str(line) + ' col ' + str(x) + '.jpg',
                            roi)
                        ...
                        for c in range(1, len(sortedContours)):
                            if cv2.contourArea(sortedContours[c]) > 30:
                                ...
                                #store mean and stdev values to analyse
                                # cv2.imwrite(segmented_char_mean_path + '/' + folder + '/ ' + str(mean)
                                #             + '.jpg', character)
                                #
                                # cv2.imwrite(segmented_char_st_dev_path + '/' + folder + '/ ' + str(stdev)
                                #             + '.jpg', character)
